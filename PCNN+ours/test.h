	...
	for (int ii = ll; ii < rr; ii++)
	{
		...
		for (int j = 0; j < relationTotal; j++) {
			...
			for (int k=0; k<bags_size; k++)
			{
				float s = 0;
				for (int i = 0; i < 3 * dimensionC; i++) 
					s += rList[k][i] * matrixRelation[r1 * 3 * dimensionC + i] * wt;//
				s = exp(s); 
				weight.push_back(s);
				weight_sum += s;
			}
			...
			for (int k = 0; k<bags_size; k++)
			{
				...
				for (int j1 = 0; j1 < relationTotal; j1++) {
					...
					for (int i1 = 0; i1 < 3 * dimensionC; i1++)
						s +=  0.5 * matrixRelation[j1 * 3 * dimensionC + i1] * zuhe_ju[k][i1];
					s += matrixRelationPr[j1];
					s = exp(s);
					tmp+=s;
					temp_res.push_back(s);
				}
				//sum[j] = max(sum[j],temp_res[j]/tmp);
				double rt= temp_res[r1]/tmp;//
				if (rt > max_s) {
					max_zuhe_index = k;
					max_s = rt;
				}
			...
			for (int j1 = 0; j1 < relationTotal; j1++) {
				float s = 0;
				for (int i1 = 0; i1 < 3 * dimensionC; i1++)
					s +=  0.5 * matrixRelation[j1 * 3 * dimensionC + i1] * zuhe_best[i1];
				s += matrixRelationPr[j1];
				s = exp(s);
				tmp+=s;
				res.push_back(s);
